...
def load_model_and_tokenizer(model_path, console_queue):
    """
    ...
    try:
        with open('models/padding.pkl', 'rb') as file:
...
def apply_model_on_rotten_tomatoes(url, model_path, console_queue):
    """
    ...
    model, tokenizer, max_len = load_model_and_tokenizer(model_path, console_queue)
...
def apply_model_on_rotten_tomatoes_thread(url, model_path, console_queue):
    """
    Avvia l'analisi del modello su Rotten Tomatoes in un thread separato.
    """
    def task():
        try:
            result = apply_model_on_rotten_tomatoes(url, model_path, console_queue)
