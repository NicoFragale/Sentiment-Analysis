...
def save_predictions(df_test, predictions, output_file, console_queue):
    df_test['predictions'] = predictions
    df_test.to_csv(output_file, index=False, encoding='iso-8859-1')
    logging.info(f"Predizioni salvate nel file '{output_file}'.")
    console_queue.put(f"Predizioni salvate nel file '{output_file}'.")

def calculate_statistics(predictions_file, console_queue):
    try:
        df_predictions = pd.read_csv(predictions_file, encoding='iso-8859-1')
...
def apply_sentiment_analysis(model_path, data_path, console_queue, tokenizer_path='models/tokenizer.pkl', padding_path='models/padding.pkl', output_file='predictions_test.csv'):
    try:
        ...
        save_predictions(df_test, predictions, output_file, console_queue)
        
        calculate_statistics(output_file, console_queue)
    except Exception as e:
...
def apply_sentiment_analysis_thread(model_path, data_path, console_queue):
    def task():
        apply_sentiment_analysis(model_path, data_path, console_queue)
